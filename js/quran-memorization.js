(() => {
  ...
  const init = () => {
    ...
    function render(){const q=normalize(search?.value);const filtered=surahs.filter(s=>{const st=getStatus(s.number);return(!q||String(s.number)===q||normalize(s.name).includes(q)||normalize(s.arabicName).includes(q))&&(activeStatus==="all"||st===activeStatus)&&(activeRevelation==="all"||s.revelation===activeRevelation);});count.textContent=`${filtered.length} surah${filtered.length===1?"":"s"}`;library.innerHTML=filtered.map(s=>{const st=getStatus(s.number);return `<article class="memorization-surah-card"><a class="surah-card-main" href="memorization-surah.html?surah=${s.number}" aria-label="Open ${s.name} memorization page"><span class="library-surah-number">${s.number}</span><span class="library-surah-copy"><span class="library-surah-arabic" lang="ar" dir="rtl">${s.arabicName}</span><strong>${s.name}</strong><small>${s.ayahCount} ayat · ${s.revelation==="makki"?"Makki":"Madani"}</small></span><span class="card-arrow" data-ui-icon="arrow-right" aria-hidden="true"></span></a><button class="library-status status-${st}" type="button" data-change-status="${s.number}" aria-label="Change ${s.name} status, currently ${labels[st]}"><i aria-hidden="true"></i>${labels[st]}</button></article>`;}).join("");hydrate(library);noResults.hidden=filtered.length!==0;}
